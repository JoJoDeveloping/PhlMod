...
public class GuiEditKasse extends GuiContainer {

	public GuiTextField[] textfields = new GuiTextField[4];
	
	public GuiEditKasse(InventoryPlayer invPlayer, TileEntityKasse kasse) {
		super(new ContainerEditKasse(invPlayer, kasse));
	}
	...
	protected void drawGuiContainerForegroundLayer(int p_146979_1_, int p_146979_2_) {
		for(int i = 0; i < 4; i++){
			if(!textfields[i].isFocused())textfields[i].setText((((ContainerEditKasse)this.inventorySlots).getPrice(i)/*==0?"Nichts verfügbar":((ContainerKasse)this.inventorySlots).prices[i]*/)+"");
